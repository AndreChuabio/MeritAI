...
# Shown as the final paragraph of the dossier PDF, in the existing
# "disclaimer" ParagraphStyle defined by dossier_mod._make_styles(). Mirrors
# the UI-facing text in web/components/LegalDisclaimer.tsx -- keep both in
...
def build_dossier(user_id: str, session_id: Optional[str] = None) -> bytes:
    ...
    with trace.step(sid, "evidence_dossier", user_id=user_id) as ctx:
        ...
        try:
            ...
            # Final paragraph on the last page, reusing the existing
            # "disclaimer" style (already used on the cover page) rather
